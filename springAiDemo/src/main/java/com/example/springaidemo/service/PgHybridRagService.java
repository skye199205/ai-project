...
/**
 * PostgreSQL 路混合 RAG：pgvector 语义召回 + 关键词召回 + RRF。
 */
@Service
@ConditionalOnBean(name = "pgVectorStore")
public class PgHybridRagService {
    ...
    public RagHybridAskResponse askHybrid(RagAskRequest request) {
        ...
        List<Document> vectorHits = pgVectorStore.similaritySearch(
        ...
        List<KeywordSearchResult> keywordHits = pgKeywordDocumentIndex.search(question, keywordTopK);
        ...
        return new RagHybridAskResponse(
                ...
                new RagRecallStats(
                        ...
                        keywordHits.size(),
                        fused.size(),
                        pgKeywordDocumentIndex.size()));
    }

    private String buildContext(List<FusedChunk> fused) {
        if (fused.isEmpty()) {
            return "（未检索到相关片段，请说明 PostgreSQL 知识库中暂无可用上下文。）";
        }
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < fused.size(); i++) {
            FusedChunk chunk = fused.get(i);
            sb.append("【片段 ").append(i + 1).append("｜来源 ").append(chunk.recallPath()).append("｜库 PostgreSQL】\n");
